...
extern void vibr_ldo_control(bool en);
	...
	while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		...
		if (pwr_status == IMGSENSOR_HW_POWER_STATUS_ON &&
		   ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
			...
			printk("bbl sensor_idx = %d, pin=%d, pin_state_on=%d, hw_id =%d(0:PMIC, 1:GPIO, 2:exGPIO)\n",
...
enum IMGSENSOR_RETURN imgsensor_hw_power_custom_front(
	struct IMGSENSOR_HW *phw,
	enum IMGSENSOR_SENSOR_IDX sensor_idx,
	enum IMGSENSOR_HW_POWER_STATUS pwr_status,
	char *curr_sensor_name)
{
	return imgsensor_hw_power_sequence(
		phw,sensor_idx,
		pwr_status,
		sensor_power_sequence,
		curr_sensor_name);
