...
static struct input_dev *acc_hit_input = NULL;
static struct wakeup_source hit_lock;
#endif
...
#ifdef CONFIG_ACCEL_HIT_HAPPEN_DETECT
#define HIT_THRESHED_STEP	16
#define MAX_THRESHOLD	(GRAVITY_EARTH_1000 * 4)
#define HIT_THRESHOLD	(MAX_THRESHOLD * MAX_THRESHOLD)
static unsigned int hit_setp_count[HIT_THRESHED_STEP]= {0};
unsigned int accel_hit_threshold = MAX_THRESHOLD;
unsigned int accel_interrupt_threshold = 0x60;

static void hit_step_count(unsigned int offset)
{
	int i = 0;

	for(i = HIT_THRESHED_STEP; i > 0; i--)
	{
		if(offset >= (accel_hit_threshold*i/HIT_THRESHED_STEP)*(accel_hit_threshold*i/HIT_THRESHED_STEP))
		{
			hit_setp_count[i-1]++;
			break;
		}
...
static bool is_hit_happen(int x, int y, int z)
{
	...
	bool is_hit = false;
	unsigned int xx = abs(x-pre_x);
	unsigned int yy = abs(y-pre_y);
	unsigned int zz = abs(z-pre_z);
	unsigned int tt = xx*xx + yy*yy + zz*zz;

	pre_x = x;
	pre_y = y;
	pre_z = z;

	//printk("%s:xx = %d, y = %d, z = %d\n",__func__,xx,yy,zz);
	if(first_in)	//ignore first in
	{
		first_in = false;
		return is_hit;
	}

	if(accel_hit_threshold)
	{
		hit_step_count(tt);

		if(tt > HIT_THRESHOLD)
			is_hit = true;
		else if(tt > accel_hit_threshold*accel_hit_threshold)
			is_hit = true;
	}
...
void hit_input_event_report(void)
{
	static struct timeval tm_s = {0};
	struct timeval tm_e;
	long tm_val = 0;
	...
	if(acc_hit_input)
	{
		...
		printk("bbl %s------tm_val = %ld\n",__func__,tm_val);
		if(tm_val > 1000 || tm_s.tv_sec == 0)
		{
			input_report_key(acc_hit_input, KEY_STOP, 1);
			input_sync(acc_hit_input);
			input_report_key(acc_hit_input, KEY_STOP, 0);
			input_sync(acc_hit_input);
			tm_s.tv_sec = tm_e.tv_sec;
			tm_s.tv_usec = tm_e.tv_usec;
			__pm_wakeup_event(&hit_lock, jiffies_to_msecs(5*HZ));
		}
	}
}
...
#ifdef CONFIG_ACCEL_HIT_HAPPEN_DETECT
			atomic_set(&cxt->delay, 50);
			if(is_hit_happen(x,y,z))
			{
...
#ifdef CONFIG_ACCEL_HIT_HAPPEN_DETECT
static ssize_t acc_store_threshold(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	int err = 0;

	err = sscanf(buf,"%d",&accel_hit_threshold);
...
static ssize_t acc_show_threshold(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", accel_hit_threshold);
}

static ssize_t acc_store_thresholdcount(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	int err = 0;
	int val = 0;
...
static ssize_t acc_show_thresholdcount(struct device *dev, struct device_attribute *attr, char *buf)
{
	int i =0;
	int j = 0;

	for(i = 0; i < HIT_THRESHED_STEP; i++)
		j += snprintf(buf+j, PAGE_SIZE-j, "%d\t", i);

	j += snprintf(buf+j, PAGE_SIZE-j, "\n");

	for(i = 0; i < HIT_THRESHED_STEP; i++)
		j += snprintf(buf+j, PAGE_SIZE-j, "%d\t", hit_setp_count[i]);
...
static ssize_t acc_store_eintthreshold(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	int err = 0;

	err = sscanf(buf,"%d",&accel_interrupt_threshold);
...
static ssize_t acc_show_eintthreshold(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "0x%04x\n", accel_interrupt_threshold);
}
...
#ifdef CONFIG_ACCEL_HIT_HAPPEN_DETECT
DEVICE_ATTR(hitthreshold, 0644, acc_show_threshold, acc_store_threshold);
DEVICE_ATTR(thresholdcount, 0644, acc_show_thresholdcount, acc_store_thresholdcount);
DEVICE_ATTR(eintthreshold, 0644, acc_show_eintthreshold, acc_store_eintthreshold);
#endif
...
#ifdef CONFIG_ACCEL_HIT_HAPPEN_DETECT
	&dev_attr_hitthreshold.attr,
	&dev_attr_thresholdcount.attr,
	&dev_attr_eintthreshold.attr,
#endif
	NULL
};
...
#ifdef CONFIG_ACCEL_HIT_HAPPEN_DETECT
	acc_hit_input = input_allocate_device();
	if(acc_hit_input == NULL)
	{
		printk("input allocate device fail.\n");
		goto exit_alloc_data_failed;
	}

	__set_bit(KEY_STOP, acc_hit_input->keybit);
	input_set_capability(acc_hit_input, EV_KEY, KEY_STOP);
	acc_hit_input->id.bustype = BUS_HOST;
	acc_hit_input->name = "acc_hit";
	err = input_register_device(acc_hit_input);
	if(err)
	{
		printk("register input device failed (%d)\n",err);
		input_free_device(acc_hit_input);
	}

	wakeup_source_init(&hit_lock, "hit wakelock");
#endif
	return 0;
...
#ifdef CONFIG_ACCEL_HIT_HAPPEN_DETECT
	input_unregister_device(acc_hit_input);
#endif
}
