...
static wait_queue_head_t usb_device_queue;
static bool usb_device_state = 0;
...
#else
u32 sw_deboun_time = 100;// 400->100,bbl
#endif
...
out:
	...
	if(host_mode == 0) //device
	{
		printk("bbl--usb switch device over,wake up wait process\n");
		usb_device_state = 1;
		wake_up_interruptible(&usb_device_queue);
	}
}

extern void usb_mode_swith_by_id(bool en);
void set_usb_to_device_mode(void)
{
	bool host_mode = musb_is_host();

	printk("bbl--usb switch device start %d\n", host_mode);
	if(host_mode)
	{
		usb_device_state = 0;
		usb_mode_swith_by_id(1);
		wait_event_interruptible_timeout(usb_device_queue, usb_device_state, 5*HZ);
		printk("bbl--usb switch device over %d\n", host_mode);
	}
 }
...
void mt_usb_otg_init(struct musb *musb)
{
	...
	init_waitqueue_head(&usb_device_queue);
	usb_device_state = 0;
	iddig_int_init();
	vbus_control = 1;
#endif
