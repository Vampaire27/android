...
static void __mt_gpufreq_clock_switch(unsigned int freq_new)
{
	...
	if (g_parking) {
		/* mfgpll_ck to clk26m */
		/*__mt_gpufreq_switch_to_clksrc(CLOCK_SUB);*/
		/* mfgpll_ck to clk364m */
		__mt_gpufreq_switch_to_clksrc(CLOCK_D3);
...
static void __mt_gpufreq_switch_to_clksrc(enum g_clock_source_enum clksrc)
{
	...
	if (clksrc == CLOCK_MAIN) {
		...
		gpufreq_pr_debug("@%s: switch to sub clock source done\n",
			__func__);
	} else if (clksrc == CLOCK_D3) {
		clk_set_parent(g_clk->clk_mux, g_clk->clk_syspll_d3);
		gpufreq_pr_debug("@%s: switch to syspll_d3 clock source done\n",
			__func__);
	} else {
		gpufreq_pr_debug(
...
static int __mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	g_clk->clk_syspll_d3 = devm_clk_get(&pdev->dev, "clk_syspll_d3");
	if (IS_ERR(g_clk->clk_syspll_d3)) {
		gpufreq_perr("@%s: cannot get clk_syspll_d3\n", __func__);
		return PTR_ERR(g_clk->clk_syspll_d3);
	}
