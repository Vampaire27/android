...
#ifndef CONFIG_FPGA_EARLY_PORTING
extern void lcm_blacklight_enable_pin_set(bool en);
static void disp_pwm_set_drverIC_en(enum disp_pwm_id_t id, int enabled)
{
	if (id == DISP_PWM0)
		lcm_blacklight_enable_pin_set(enabled?1:0);
}

extern void lcm_disp_pwm_control(bool is_pwm);
	...
	enum disp_pwm_id_t id, int enabled)
{
	...
	lcm_disp_pwm_control(enabled?1:0);
