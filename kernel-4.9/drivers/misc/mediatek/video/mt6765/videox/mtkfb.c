...
#endif
static int gScreen_type = 0;
int vcom_pwm_level = 0;
int avdd_pwm_level = 0;
...
static int get_cmd_srceen_type(void)
{
	int duty = 0;
	char *p = NULL;
	p = strstr(saved_command_line, "gScreen_type=");
	if(p != NULL)
	{
		if(p[14] == ' ')
			duty = (int)(p[13] - '0');
		else
			duty =(int) ((p[13] - '0') * 10 + p[14] - '0');

		printk("[weiwei.guo] gScreen_type = %d\n",duty);
	}

	return duty;

}
static int get_cmd_vcom_duty(void)
{
	int duty = 0;
	char *p = NULL;
	p = strstr(saved_command_line, "gVcom_duty=");
	if(p != NULL)
	{
		if(p[11] == '-')
			duty = 34;
			...
			duty =(int) ((p[11] - '0') * 10 + p[12] - '0');
		else
			duty = 100;

		printk("[weiwei.guo] gVcom_duty_kernel = %d\n",duty);
	}
...
static int get_cmd_avdd_duty(void)
{
	int duty = 0;
	char *p = NULL;
	p = strstr(saved_command_line, "gAvdd_duty=");
	if(p != NULL)
	{
		if(p[11] == '-')
			duty = 20;
		...
		printk("[weiwei.guo] gAvdd_duty_kernel = %d\n",duty);
	}
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	gScreen_type = get_cmd_srceen_type();
	if(gScreen_type == 0)
		gScreen_type = 10;

	vcom_pwm_level = get_cmd_vcom_duty();
	if(vcom_pwm_level == 0)
		vcom_pwm_level = 48;

	avdd_pwm_level = get_cmd_avdd_duty();
	if(avdd_pwm_level == 0)
		avdd_pwm_level = 32;
...
static void mtkfb_late_resume(void)
{
	...
	lcm_vcom_set(vcom_pwm_level);
	lcm_avdd_set(avdd_pwm_level);
