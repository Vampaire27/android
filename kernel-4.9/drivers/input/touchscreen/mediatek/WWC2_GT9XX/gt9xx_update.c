...
#define CONFIG_FILE_PATH_1					"/custom/goodixTpConfig.cfg"
...
u8 gup_uninit_update_proc_custom(void);
...
static s8 gup_update_config(struct i2c_client *client)
{
		...
		if (ret < 0) {
            gtp_update = 2;//Add by liuxin,read config file fail!!
			GTP_ERROR("[update_cfg]Read config file failed.");
            
            gup_uninit_update_proc_custom();
		...
		for (i = 0, file_cfg_len = GTP_ADDR_LENGTH; i < count; i += 5) {
			if ((buf[i] == '0') && ((buf[i+1] == 'x') || (buf[i+1] == 'X'))) {
				...
				if ((high == 0xFF) || (low == 0xFF)) {
					ret = 0;
                    gtp_update = 4;//Add by liuxin,Illegal config file!!
        ...
        for (i = 0; i < file_cfg_len-4; i++) {
			checksum +=
			    (file_config+2)[i];            
		}
        if((((~checksum+1)&0xFF) - (file_config+2)[file_cfg_len-4]) == 0 && (((file_config+2)[file_cfg_len-3] - 1) == 0))
        {
            if(i2c_read_bytes(client,0x8047,buf8047,2) ==0)
            {
                printk("cfg ver = 0x%x\n",*(file_config+2));
                printk("reg8047 value= 0x%x\t 0x%x\n",buf8047[1],buf8047[0]);
                if((*(file_config+2) == 0x00) || (((*(file_config+2)) - buf8047[0]) > 0))
                {
                    i = 0;
	                while (i++ < 5) {
		                ret = gup_i2c_write(client, file_config, file_cfg_len);
		                if (ret > 0) {
                            gtp_update = 6;//Add by liuxin,Send config SUCCESS!!
			                GTP_ERROR("[update_cfg]Send config SUCCESS.\n");
                            gup_uninit_update_proc_custom();
			                break;
	                }
                    gtp_update = 3;//Add by liuxin,i2c error!!
                ...
                {
                    gtp_update = 5;//Add by liuxin,Version too low,can not update!!!
                    GTP_ERROR("[update_cfg]Version too low,can not update!\n");
                    gup_uninit_update_proc_custom(); 
                }
            }
            else
...
static void gup_search_file(s32 search_type)
{
		...
		for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i) {
			...
			if ((search_type & AUTO_SEARCH_CFG) && !(got_file_flag & CFG_FILE_READY)) {
                ...
                {
                    gtp_update = 2;//Add by liuxin,read config file fail!!
                    GTP_ERROR("[update_cfg]NULL config file.\n");
                    gup_uninit_update_proc_custom();
                }
				if (IS_ERR(pfile)) {
...
u8 gup_init_update_proc_custom(void)
{

	printk("[update_proc] Ready to run auto update thread\n");
	
	if(tpd_load_status != 1)
	{
		printk("[update_proc] GT9XX not found, exit the update process directly\n");
		return -1;
	}

	thread_update = kthread_run(gup_update_proc, (void *)NULL, "guitar_update");
...
u8 gup_uninit_update_proc_custom(void)
{
    printk("Ready to kill auto update thread\n");
