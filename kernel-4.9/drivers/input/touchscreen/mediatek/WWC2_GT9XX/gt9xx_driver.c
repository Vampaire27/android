...
#include "tpd.h"
#include "include/gt9xx_config.h"
...
/*Begin:add by liuxin 20180414---use for acc func to disable tp irq & enable tp irq,reviewed by fjie*/
void tpd_on_extern(void);
void tpd_off_extern(void);
/*end:add by liuxin 20180414---use for acc func to disable tp irq & enable tp irq,reviewed by fjie*/
#ifdef CONFIG_GTP_CHARGER_DETECT
...
#if (defined(CONFIG_GTP_ESD_PROTECT) && !defined(CONFIG_GTP_CHARGER_DETECT))
...
#ifdef CONFIG_GO_TO_RECOVERY_MODE
#include <mtk_reboot.h>
struct point {
	u32 x;
	u32 y;
};
#define AREA_SIZE_X	50
#define AREA_SIZE_Y	40
#define MAX_TOUCH_COUNT 11
#define COUNT_TIME	2*HZ
static struct point start_point = {.x = 1024 - AREA_SIZE_X,
							   .y = 0};
static struct timer_list touch_count_timer;
static int touch_count = 0;
static void touch_count_timer_func(unsigned long n)
{
	...
	touch_count = 0;
	mod_timer(&touch_count_timer,jiffies+COUNT_TIME);
}

static bool touch_in_area(int x,int y)
{
	if(x > start_point.x && x <= start_point.x + AREA_SIZE_X &&\
		y > start_point.y && y <= start_point.y + AREA_SIZE_Y)
		return 1;
	else
		return 0;
}
...
int cpu_opp_id = 0;
bool force_adjust_cpufreq = 0;
#ifdef CONFIG_THERMAL_MONITOR
#define MIND_MONITOR_CYCLE	8*HZ
#define LOW_MONITOR_CYCLE		4*HZ
#define HIGH_MONITOR_CYCLE	1*HZ

#define TMEP_ARRAY_SIZE	10
#define MONITOR_TEMP		110000
#define CPU_STEP			15
#define STEP_TEMP			2000
static struct timer_list thermal_monitor_timer;
extern int tscpu_get_curr_temp(void);
extern unsigned int get_cur_cpu_freq(unsigned int cpu_id);
extern unsigned int get_max_cpu_freq(unsigned int cpu_id);


static int get_thermal_level(int temp)
{
	static int temp_array[TMEP_ARRAY_SIZE] = {0};
	int average_temp = 0;
	int sum = 0;
	int i = 0;
	int level = 0;

	for(i = TMEP_ARRAY_SIZE-1; i > 0; i--)
	{
		temp_array[i] = temp_array[i-1];
	}
	temp_array[0] = temp;

	for(i = 0; i < TMEP_ARRAY_SIZE; i++)
	{
		if(temp_array[i] == 0)
			temp_array[i] = MONITOR_TEMP;

		sum+= temp_array[i];
	}
	average_temp = sum/TMEP_ARRAY_SIZE;

	for(level = 0; level < CPU_STEP; level++)
	{
		if(average_temp <  MONITOR_TEMP+level*STEP_TEMP)
			break;
	}
...
static void thermal_monitor_timer_func(unsigned long n)
{
	int temp = tscpu_get_curr_temp();
	int level = get_thermal_level(temp);
	unsigned long monitor_cycle =  MIND_MONITOR_CYCLE;
	unsigned int current_cpu0freq = get_cur_cpu_freq(0);
	unsigned int current_cpu4freq = get_cur_cpu_freq(4);
	unsigned int max_cpu0freq = get_max_cpu_freq(0);
	unsigned int max_cpu4freq = get_max_cpu_freq(4);

	cpu_opp_id = level;
	printk("bbl temp = %d,level = %d,cpu_opp_id = %d,cur_cpu0freq = %dkHz,cur_cpu4freq = %dkHz\n",temp,level,cpu_opp_id,current_cpu0freq,current_cpu4freq);

	if(level == 0)
	{
		if(temp >=  MONITOR_TEMP - 10000)
			monitor_cycle = LOW_MONITOR_CYCLE;
		else
			monitor_cycle =  MIND_MONITOR_CYCLE;
	}
	else
		monitor_cycle = HIGH_MONITOR_CYCLE;

	if(level > 0 && (max_cpu0freq == current_cpu0freq || max_cpu4freq == current_cpu4freq))
		force_adjust_cpufreq = 1;
	else
		force_adjust_cpufreq = 0;

	mod_timer(&thermal_monitor_timer,jiffies+monitor_cycle);
}
#endif

static struct proc_dir_entry *gt91xx_config_proc;

extern int ctp_switch;
...
#endif //#ifdef TPD_REFRESH_RATE

/* ============================================================= */
//BEGIN: add by fangjie for JNI get touchpanel position
#ifdef JNI_GET_POSITION

static int gJNI_position_x = 0;
static int gJNI_position_y = 0;

static ssize_t show_position_X(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gJNI_position_x);
}

static ssize_t store_position_X(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	return size;
}
static ssize_t show_position_Y(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gJNI_position_y);
}

static ssize_t store_position_Y(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(tpd_position_X, 0664, show_position_X, store_position_X);
static DEVICE_ATTR(tpd_position_Y, 0664, show_position_Y, store_position_Y);



static struct device_attribute *gt9xx_attrs[] = {
	//&dev_attr_tpd_refresh_rate,
    	&dev_attr_tpd_position_X,
	&dev_attr_tpd_position_Y,
};
#endif
//END: add by fangjie for JNI get touchpanel position
/*=============================================================*/
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	GTP_DEBUG("fj : before abs_x_max = %d,%d\n", abs_x_max, abs_y_max);
	GTP_DEBUG_FUNC();
	if ((abs_x_max == 0) && (abs_y_max == 0)) {
		//BEGIN: modify by fangjie
		//abs_x_max =  (config[RESOLUTION_LOC + 1] << 8) + config[RESOLUTION_LOC];
		//abs_y_max = (config[RESOLUTION_LOC + 3] << 8) + config[RESOLUTION_LOC + 2];
		abs_x_max =  GTP_MAX_WIDTH;
		abs_y_max = GTP_MAX_HEIGHT;		
		//END: modify by fangjie
		int_type = (config[TRIGGER_LOC]) & 0x03;
	}
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
		...
		while (retry++ < 5) {
			...
			GTP_DEBUG("gtp_i2c_test: GTP_REG_HW_INFO : %08X", hw_info);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	if (gtp_resetting)
	{
		GTP_INFO("GTP RESET again, return !\n");
		return;
	}
...
reset_proc:
	...
	//GTP_ERROR("turn on power reg-vldo28\n");
	ret = regulator_enable(tpd->reg);
	if (ret != 0)
	{
		TPD_DMESG("Failed to disable reg-vldo28: %d\n", ret);
		return -1;
	}
#endif
...
static int tpd_irq_registration(void)
{
	...
	//node = of_find_compatible_node(NULL, NULL, "mediatek,mt6739-touch");//0704
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GTP_DEBUG("tpd_i2c_probe addr: %x\n",client->addr); //fangjie temp add
	...
	if (ret < 0) {
		...
		tpd_load_status = 1; //set tpd load always right to avoid booting fail when no /dev/intput 
	#endif	
		GTP_ERROR("I2C communication ERROR!");
		goto out; //fangjie temp close 
	}
	...
	return 0;
out: //fangjie temp close for debug
	gtp_free_gpio_res();
#ifdef CONFIG_GTP_POWER_CTRL_SLEEP
	gtp_gpio_output(GTP_RST_GPIO, 0);
	msleep(20);
	...
	if (ret_p != 0)
		GTP_INFO("Failed to disable reg-vgp6: %d\n", ret_p);
...
void force_reset_guitar(void)
{
	...
	ret = regulator_enable(tpd->reg);
	if (ret != 0)
		GTP_INFO("Failed to enable reg-vgp6: %d\n", ret);
	msleep(30);
...
#ifdef CONFIG_GO_TO_RECOVERY_MODE
		if(touch_in_area(tpd_history_x,tpd_history_y))
			touch_count++;
#endif
		tpd_history_x = 0;
		tpd_history_y = 0;
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		GTP_DEBUG("key_value = %d, pre_key=%d",key_value,pre_key);
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				//GTP_DEBUG("fj: abs:%d %d\n",abs_x_max, abs_y_max);
				//GTP_DEBUG("fj: before: %d,%d\n", input_x,input_y);
				if((input_x > GTP_MAX_WIDTH ) || (input_y > GTP_MAX_HEIGHT) || (input_x == 0))
				{
/*Begin:modify by liuxin for resolve TP issue 20180411---touch the left margin but actually show the right margin,the solution is handle the region between key & screen to tpd_up no tpd_down */
                    #ifdef CONFIG_CH001_KS_SUPPORT
                    if((input_x < GTP_KEY_X_POSITION_DTS - GTP_X_GAP_BETWEEN_SCREEN_AND_KEY) && (input_x > GTP_MAX_WIDTH))
                    {
                        tpd_up(0,0,0);
                        input_sync(tpd->dev);
                    }
                    ...
                    #endif
/*End:modify by liuxin for resolve TP issue 20180411---touch the left margin but actually show the right margin */
                    {    					
                        GTP_DEBUG("touch the virtual Key region!");
                    }
				}
				else 
				//END:modify by fangjie
				{
		                    if (ctp_switch & 0x1)
				    {
                                         //GTP_DEBUG("fj ctp X switch \n");
					 input_x = abs_x_max - input_x;                                        
				    }
				    if (ctp_switch & 0x2)
				    {
				         //GTP_DEBUG("fj ctp Y switch \n");
				         input_y = abs_y_max - input_y;
		 		    }			

				}
				//GTP_DEBUG("fj: after: %d, %d\n", input_x,input_y);
                #ifdef GOODIX_TP_LINGFEI
				if(0 == input_x)
                {
                    input_x = 0x600;//,modify by liuxin,remapping to distinguish left key region
                }
                #endif
#ifdef CONFIG_GTP_WITH_HOVER
	                id = coor_data[0];
...
static int tpd_local_init(void)
{
	...
	gpDMABuf_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, GTP_DMA_MAX_TRANSACTION_LENGTH,
...
#endif
	//BEGIN: delete by fangjie , because the VDD supply from MCU board.
	#ifdef CONFIG_GTP_POWER_CTRL_SLEEP
	...
	#endif
	//END: delete by fangjie , because the VDD supply from MCU board.
	...
	tpd_type_cap = 1;
#ifdef CONFIG_GO_TO_RECOVERY_MODE
	memset((void *)&touch_count_timer, 0, sizeof(touch_count_timer));
	touch_count_timer.expires = jiffies + COUNT_TIME;	/* wait 2s to stable */
	touch_count_timer.function = touch_count_timer_func;
	touch_count_timer.data = 0;
	init_timer(&touch_count_timer);
	add_timer(&touch_count_timer);
#endif

#ifdef CONFIG_THERMAL_MONITOR
	memset((void *)&thermal_monitor_timer, 0, sizeof(thermal_monitor_timer));
	thermal_monitor_timer.expires = jiffies + LOW_MONITOR_CYCLE;	/* wait 10s to stable */
	thermal_monitor_timer.function = thermal_monitor_timer_func;
	thermal_monitor_timer.data = 0;
	init_timer(&thermal_monitor_timer);
	add_timer(&thermal_monitor_timer);
#endif
...
static bool suspend_flag = 0;
static void tpd_suspend(struct device *h)
{
	s32 ret = -1;

	printk("bbl tp System suspend.\n");
#ifdef CONFIG_GO_TO_RECOVERY_MODE
	del_timer(&touch_count_timer);
#endif

#ifdef CONFIG_THERMAL_MONITOR
	del_timer(&thermal_monitor_timer);
#endif
...
#ifdef CONFIG_THERMAL_MONITOR
	mod_timer(&thermal_monitor_timer,jiffies+LOW_MONITOR_CYCLE);
#endif
}
...
static struct tpd_driver_t tpd_device_driver = {
		...
		//BEGIN: add by fangjie for JNI get touchpanel position
		#ifdef JNI_GET_POSITION
		.attrs = { ///sys/bus/platform/drivers/mtk-tpd/touch #tpd_position_X
		  .attr = gt9xx_attrs,
...
/*Begin:add by liuxin 20180508---add reset operation in tpd_on_extern func to avoid i2c err when acc on,reviewed by fjie*/
/*Begin:add by liuxin 20180414---use for acc func to disable tp irq & enable tp irq,reviewed by fjie*/
void tpd_off_extern(void)
{
	printk("%s--suspend_flag = %d\n",__func__,suspend_flag);
...
void tpd_on_extern(void)
{
	...
	if(suspend_flag)	//tpd in suspend
		return;
...
/*end:add by liuxin 20180414---use for acc func to disable tp irq & enable tp irq,reviewed by fjie*/
/*end:add by liuxin 201800508---add reset operation in tpd_on_extern func to avoid i2c err when acc on,reviewed by fjie*/
