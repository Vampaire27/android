...
static int getNextFile(const char *fileDirectory, RECORD_MODE mode, enum CAMERA_FORMAT format)
{
	...
	else
	{
		...
		{
			case FRONT_RECORD:
				strftime(name, sizeof(name), "front_540P_%F_%T.ts",p);
				break;
			case BACK_RECORD:
				strftime(name, sizeof(name),"back_540P_%F_%T.ts",p);
				break;
			case LEFT_RECORD:
				strftime(name, sizeof(name),"left_540P_%F_%T.ts",p);
				break;
			case RIGHT_RECORD:
				strftime(name, sizeof(name),"right_540P_%F_%T.ts",p);
				break;
			case QUART_RECORD:
...
static int wwc2_record_four_with_audio(struct WWC2_FOUR_RECORD_THREAD *t, pthread_mutex_t* mutex, struct PR2100_RECORD *obj)
{
	...
	int frameRateFps = 25;
	int width = (cameraFormat == CH0FHD_CH1FHD)?1920:960;
	int height = (cameraFormat == CH0FHD_CH1FHD)?1080:540;
	int bitRateBps = obj->record_bps >> 1;
