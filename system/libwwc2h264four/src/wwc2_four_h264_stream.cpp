...
#include <semaphore.h>
#include <pthread.h>
#include "wwc2_four_h264_stream.h"

using namespace android;

struct WWC2_FOUR_H264_STREAM_THREAD {
	bool threadLoopFlag;
	unsigned char *yuvData;
	const unsigned char *threadName;
	const unsigned char *socketName;
	const unsigned char *shareStreamFileName;
	H264_MODE	mode;
	int fd;
	int streamFd;
	pthread_t thread;
...
class wwc2CameraSourceH264Four : public MediaSource {

public:
	wwc2CameraSourceH264Four(int width, int height, void *data, int fps, int colorFormat, int h264CmdFd, H264_MODE mode, struct WWC2_FOUR_H264_STREAM_THREAD* thread, sem_t* sem)
		:mWidth(width),
		mHeight(height),
		mFrameRate(fps),
		mColorFormat(colorFormat),
		mH264CmdFd(h264CmdFd),
		mH264Mode(mode),
		mThread(thread),
		msem(sem),
	...
	virtual status_t read(MediaBuffer **buffer, const MediaSource::ReadOptions *options __unused) {
		...
		{
			::read(mH264CmdFd, &recv, 1);
			//ALOGD("bbl--h264 recv = %d", recv);
			if(recv == 0xff)
			{
...
protected:
    virtual ~wwc2CameraSourceH264Four() {}
...
private:
    ...
    int mH264CmdFd;
    H264_MODE mH264Mode;
	struct WWC2_FOUR_H264_STREAM_THREAD* mThread;
	sem_t* msem;
    size_t mSize;
    ...
    wwc2CameraSourceH264Four(const wwc2CameraSourceH264Four &);
    wwc2CameraSourceH264Four &operator=(const wwc2CameraSourceH264Four &);
};

void wwc2CameraSourceH264Four::sem_wait_rt(sem_t *pSem, nsecs_t reltime, H264_MODE mode)
{
	struct timespec ts;
...
int wwc2_four_h264_stream_internal(void* pStream, int socketFd, unsigned int fps, unsigned int bitRate)
{
	...
	char saveDirectory[PROPERTY_VALUE_MAX] = {'\0'};
	status_t err = OK;
	sem_t h264StreamStartSem;
	struct WWC2_FOUR_H264_STREAM_THREAD* thread = (struct WWC2_FOUR_H264_STREAM_THREAD*)pStream;
	H264_MODE h264Mode = (H264_MODE)thread->mode;

	sem_init(&h264StreamStartSem, 0, 0);
	android::ProcessState::self()->startThreadPool();
	wwc2_h264_set_file_directory(h264Mode, saveDirectory);
	iFileDirectory = saveDirectory;	
	sp<MediaSource> source = new wwc2CameraSourceH264Four(width, height, thread->yuvData, frameRateFps, colorFormat, socketFd, h264Mode, thread, &h264StreamStartSem);
...

	{

		{
			...
			writer->setCameraH264Mode(h264Mode);
			writer->setH264StreamStartSem(&h264StreamStartSem);
			writer->setH264StreamStopSem(&thread->stopSem);
			writer->setH264StreamFd(thread->streamFd);
			writer->setH264StreamSocketFd(socketFd);
