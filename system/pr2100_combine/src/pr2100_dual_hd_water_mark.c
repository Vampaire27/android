...
static void dual_hd_getTimeString(char* mTime)
{
	struct tm *p = NULL;
...
static void pr2100_dual_hd_water_mark(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo, DISPLAY_MODE mode, struct WATER_MARK water)
{
		...
		case 1:
			if(topLength > (position.y+sourceHeight))
			{
				for(h = 0; h < sourceHeight; h++)
				{
					...
					{
						if(*(source + sourceWidth*h + w) < 128)
							*(dstAddr +  (DUAL_HD_FRAME_WIDTH-position.x)*2 + 1 - w*2) = 0xff;
					}
				}
			}
			else if(topLength <= position.y)
...
static void pr2100_dual_hd_time_water_mark(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo, DISPLAY_MODE mode, char *timeData)
{
	unsigned int i = 0;
	struct WATER_MARK water = {.width = 20, .height = 32, .position = {.x = 40, .y = DUAL_HD_FRAME_HEIGHT - 80}};
	...
	for(i = 0; i < 20; i++)
	{
		...
		if(water.data)
		{
			pr2100_dual_hd_water_mark(frameInfo,pFrameInfo, mode, water);
		}
	}
}

static void pr2100_dual_hd_card_water_mark(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo, DISPLAY_MODE mode, int cardData[10])
{
	unsigned int i = 0;
...
static void pr2100_dual_hd_gps_water_mark(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo, DISPLAY_MODE mode, char gps[32])
{
	unsigned int i = 0;
	struct WATER_MARK water = {.width = 20, .height = 32, .position = {.x = DUAL_HD_FRAME_WIDTH - 480, .y = 40}};
...
static void pr2100_dual_hd_water_mark_channel(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo)
{
	struct WATER_MARK ch0Water = {.data = waterMarkFBLR_40x40[0], .width = 40, .height = 40, .position = {.x = DUAL_HD_FRAME_WIDTH - 80, .y = DUAL_HD_FRAME_HEIGHT - 80}};
	struct WATER_MARK ch1Water = {.data = waterMarkFBLR_40x40[1], .width = 40, .height = 40, .position = {.x = DUAL_HD_FRAME_WIDTH - 80, .y = DUAL_HD_FRAME_HEIGHT - 80}};

	if( pr2100Obj->channelWaterMark)
	{
		pr2100_dual_hd_water_mark(frameInfo, pFrameInfo, FRONT_DISPLAY, ch0Water);
		pr2100_dual_hd_water_mark(frameInfo, pFrameInfo, BACK_DISPLAY, ch1Water);
	}
}

static void pr2100_dual_hd_water_mark_time(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo)
{
	char timeData[21] = {0};

	if( pr2100Obj->timeWaterMark)
	{
		dual_hd_getTimeString(timeData);
		pr2100_dual_hd_time_water_mark(frameInfo, pFrameInfo, FRONT_DISPLAY, timeData);
		pr2100_dual_hd_time_water_mark(frameInfo, pFrameInfo, BACK_DISPLAY, timeData);
	}
}

static void pr2100_dual_hd_water_mark_card(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo, int cardData[10])
{
	if(pr2100Obj->cardWaterMark)
	{
		pr2100_dual_hd_card_water_mark(frameInfo, pFrameInfo, FRONT_DISPLAY, cardData);
		pr2100_dual_hd_card_water_mark(frameInfo, pFrameInfo, BACK_DISPLAY, cardData);
	}
}

static void pr2100_dual_hd_water_mark_gps(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo, char gps[32])
{
	if(pr2100Obj->gpsWaterMark)
	{
		pr2100_dual_hd_gps_water_mark(frameInfo, pFrameInfo, FRONT_DISPLAY, gps);
		pr2100_dual_hd_gps_water_mark(frameInfo, pFrameInfo, BACK_DISPLAY, gps);
	}
}

void pr2100_dual_hd_water_mark_custom(struct DUAL_HD_FRAME_INFO *frameInfo, struct DUAL_HD_FRAME_INFO *pFrameInfo, int cardData[10], char gps[32])
{
	pr2100_dual_hd_water_mark_channel(frameInfo, pFrameInfo);
	pr2100_dual_hd_water_mark_time(frameInfo, pFrameInfo);
	pr2100_dual_hd_water_mark_card(frameInfo, pFrameInfo,cardData);
	pr2100_dual_hd_water_mark_gps(frameInfo, pFrameInfo,gps);
}
